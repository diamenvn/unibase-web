...
<script>
    ...
    const api = {
        getOrderList: function(callback) {
            stt = 0;
            lib.send.get('{{route("api.report.reportBillingForSaleFromMkt")}}', callback, window.location.search);
        }
    }
    ...
    const activity = {
        ...
        setTime: function() {
            setTimeout(function(){
                ...
                $('#title').html('Báo cáo doanh thu sale từ ngày ' + start[2] + '/' + start[1] + '/' + start[0] + ' đến ' + end[2] + '/' + end[1] + '/' + end[0]);
