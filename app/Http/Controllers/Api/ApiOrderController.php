...
class ApiOrderController extends Controller
{
  ...
  {
    $user = $this->user->info();
    $columns = $this->order->getListLabelByCompanyId($user->company_id)->load('order');
    if ($columns) {
  ...
  public function getAllListOrder(Request $request)
  {
    ...
    $request['label_ids'] = $this->order->getListLabelByCompanyId($user->company_id)->pluck('_id')->toArray();
    $isPaginate = (isset($request['paginate']) && $request['paginate']);
    $lists = $this->order->search($request)
      ->select($this->select_order_fields)
      ->with('product')
      ->with('source')
      ->with('reciver')
      ->with('care')
  ...
  public function updateOrder($order, $request)
  {
    if ($this->user->isMkt()) {
      ...
      if($order->is_order_care && empty($order->user_care_id)) {
        $data['user_care_id'] = $this->user->info()->_id;
      }
    }
  ...
  public function newOrderShip($order, $request)
  {
    ...
    if (empty($order->load('ship')->ship) || $this->user->isUserSale() || $this->user->isCare()) {
    ...
    if (($this->user->isSale() || $this->user->isCare()) && $request['reason'] == "success" && empty($order->load('ship')->ship->date_success_order)) {
