...
<script>
  const api = {
    getOrderList: function(callback) {
      lib.send.get('{{route("api.lading.getListCare")}}', callback, window.location.search);
    }
  }
  ...
  const html = {
    renderList: function(res, item) {
      userReciver = product = '';
      if (item.order.care) {
        userReciver = '<span class="d-inline-block mb-1 user--reciver mr-2">' + item.order.care.name + '<i class="fas fa-star ml-1 fs-12 text-warning"></i></span>';
      }
      date = '';
  ...
  $(function() {
    ...
    $(document).on('click', '.btn-click-save-modal-js', function() {
      ...
      lib.send.post('{{route("api.lading.updateStatusOrderCare")}}', function(res) {
        if (res.success) {
