...
use App\Model\Mongo\OrderCareModel;
use Exception;
use Carbon\Carbon;

class OrderCareService
{

    public function __construct(OrderCareModel $orderCareModel, CompanyProductModel $product, CompanySourceModel $source, ListProductModel $listProduct)
    {
        $this->result = $orderCareModel;
    ...
    public function search($data)
    {
        ...
        if (isset($data['user_care_id'])) {
            if(is_array($data['user_care_id'])){
                $query = $query->where(function($query) use ($data){
                    foreach ($data['user_care_id'] as $care) {
                        if ($care == null) {
                            $query = $query->orWhereNull('user_care_id');
                        }else{
                            $query = $query->orWhere('user_care_id', $care);
                        }
                    }
                });
            }else{
                $query = $query->where('user_care_id', $data['user_care_id']);
            }
        }
